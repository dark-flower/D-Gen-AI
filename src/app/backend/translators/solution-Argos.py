...
def answer_user_question(question:str="this is a dummy question, don't answer it", source_lang:str='en'):
    ...
    if source_lang == 'ar':
        user_prompt = argostranslate.translate.translate(question, to_code, from_code)

    output = qa_lm({'query': user_prompt})

    if source_lang == 'ar':
        output['response'] = argostranslate.translate.translate(output["result"],from_code, to_code)
    else:
